...
| U7 | Polish + production build | ✅ done — cqserver's admin HTTP server now mounts the admin-ui dist under `/ui/*` via `tower-http` `ServeDir` with SPA fallback to `index.html`. Vite `base: '/ui/'` in production builds; React Router uses `basename={import.meta.env.BASE_URL}`. Admin-ui's API client auto-detects: when served from `/ui/*` it talks to the same origin's root; in dev mode it still goes through `/admin-api/*`. `CQSERVER_ADMIN_UI_DIR` env var lets operators point at any dist location. End-to-end verified: `/ui/`, `/ui/topics`, hashed assets, JSON APIs all serve from the same release binary. Docs: `docs/admin-ui.md` covers dev + deploy + page-by-page reference. |
| U7-fw | Windows-style hotkeys + code splitting | ✅ done — `src/lib/keyboard.tsx` provider with document-level keydown dispatch and a registered-shortcut registry. **Ctrl+K** command palette (fuzzy search across routes / live topics / actions). **Ctrl+/** cheat sheet (also reachable from a header keyboard button). **F5** invalidates every active TanStack Query. **Ctrl+F** focuses the page filter input (`data-page-filter` on Topics / Subscriptions / Metrics / Config). **Alt+1..9** jump to sidebar items with on-hover hints. **Esc** closes any modal. Modals auto-close on route change. Pages now `React.lazy()`-loaded with Suspense fallbacks; AG-Grid + recharts moved to named manualChunks so first-paint dropped from ~300 KB → **116 KB gzipped** (60% reduction). |
