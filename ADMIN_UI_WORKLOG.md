...
| U7 | Polish + production build | ✅ done — cqserver's admin HTTP server now mounts the admin-ui dist under `/ui/*` via `tower-http` `ServeDir` with SPA fallback to `index.html`. Vite `base: '/ui/'` in production builds; React Router uses `basename={import.meta.env.BASE_URL}`. Admin-ui's API client auto-detects: when served from `/ui/*` it talks to the same origin's root; in dev mode it still goes through `/admin-api/*`. `CQSERVER_ADMIN_UI_DIR` env var lets operators point at any dist location. End-to-end verified: `/ui/`, `/ui/topics`, hashed assets, JSON APIs all serve from the same release binary. Docs: `docs/admin-ui.md` covers dev + deploy + page-by-page reference. **Polish items (⌘K palette, vim-style nav, Suspense routing) explicitly deferred** — left as small independent follow-ups. |
