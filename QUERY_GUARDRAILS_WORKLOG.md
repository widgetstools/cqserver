...
| G4 | Runtime caps + observability | ✅ done — `hard_max_sow_result_rows`/`bytes` enforced inside `deliver_streaming_snapshot` on both JSON-fast-path and slow-path. Aborts mid-stream with an error frame naming the cap, then `group_end`. Metrics: `cq_query_runtime_capped_total{topic}`, `cq_sow_actual_rows`, `cq_sow_actual_bytes` (histograms for estimate-vs-actual visibility). Integration test `tcp_g4_runtime_row_cap_aborts_sow`. |
| G5 | Per-user query budgets | ✅ done — `cq_transport::auth::QueryBudget` (all-Option struct) on `User`; `merge_with(server_limits)` picks the tighter side per field so per-user overrides can only TIGHTEN; `AuthStore::query_budget_for(username)`; subscribe-time + runtime-cap paths consult the effective merged limits. `[auth.users.query_budget]` TOML block. 5 unit tests covering tighten / loosen-rejected / zero-as-disabled / server-disabled-but-user-set / User round-trip. |
