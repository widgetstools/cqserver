// AG Grid v33+ Theming API factory — ESM port of
// `staruidesign1/aggrid-theme.js`. The Stockflux design system ships
// a (palette × mode) matrix of colour packs; this module builds and
// caches a `themeQuartz.withPart(iconSetQuartzBold).withParams(...)`
// theme object for each combination on demand.
//
// Use:
//   import { getAgGridTheme } from '@/lib/aggrid-theme';
//   <AgGridReact theme={getAgGridTheme('teal', 'dark')} ... />

import { themeQuartz, iconSetQuartzBold, type Theme } from 'ag-grid-community';
import type { Palette } from '@/components/theme/ThemeProvider';

export type AgGridMode = 'dark' | 'light';

// ── Shared params (typography, spacing, row dynamics) ──
const shared = {
  iconSize: 13,
...
const palettes: Record<Palette, Record<AgGridMode, ColorPack>> = {
  teal: {
    dark: {
      bg: '#0d1219', fg: '#f1f4f6',
  ...
  grey: {
    dark: {
      bg: '#0c0c0d', fg: '#f8f9fa',
      chrome: '#141517', header: '#1a1b1d', headerText: '#a1a2a5',
      odd: '#0f0f11', hover: 'rgba(248,249,250,0.04)', sel: 'rgba(248,249,250,0.08)',
...
export function getAgGridTheme(palette: Palette = 'teal', mode: AgGridMode = 'dark'): Theme {
  const key = `${palette}-${mode}`;
  if (cache[key]) return cache[key];
  const pack = palettes[palette]?.[mode] ?? palettes.teal.dark;
  return (cache[key] = buildTheme(pack));
}
