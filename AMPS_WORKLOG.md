...
**Status:** ✅ done (single-source views + S20-followup INNER JOIN USING views)
...
- 2026-05-24 — **S20 JOIN-view follow-up done** (INNER JOIN USING with two-topic refresh fan-in):
  - **`cq_core::query`** — `ParsedQuery` gains an optional `JoinSpec { right_topic, using }`; `parse_join_clause` accepts a single `[INNER] JOIN <topic> USING (col, ...)` clause from sqlparser's AST. LEFT/RIGHT OUTER, ON-clause expressions, and multi-table chains are rejected at parse time with a clear error and tracked as the natural follow-ups. New helpers: `peek_join(sql)` returns `(left_topic, right_topic, using_columns)` without compiling the full query (server uses this to look up the right topic in the registry before parsing); `combined_join_schema(left, right, using)` synthesises the `left ∪ right` schema with USING columns appearing once, so the predicate / aggregate compiler sees every column that the post-join row exposes. Quoted topic identifiers (`"/positions"`, backtick, square-bracket) are unwrapped via a `strip_identifier_quotes` helper so config-side topic paths with `/` work without breaking the SQL grammar.
  - **`cq_core::query::execute_join_query(query, left_store, right_store)`** — streaming hash join. Builds a `HashMap<key, right_row_idx>` over the right store (using a `\x1f`-separated canonical-string key), walks the left store once, materialises every joined left-row into a temporary `ColumnStore` with the combined schema, and delegates to the existing `execute_query_with_index` for predicate / GROUP BY / aggregate / projection. Reuses all the aggregator runtime (S19) and projection code paths unchanged. INNER semantics: rows whose left or right key is NULL or has no match drop.
  - **`Topic::execute_join_query(parsed, &right_topic)`** — wires the in-process Topic-level read locks on BOTH stores for the duration of the join (read locks compose) and dispatches to the join executor.
  - **`cq_core::view::View`** — `right_topic: Option<SharedTopic>` field; `build_view_topic_joined(source, right_resolver, sql, ...)` uses `peek_join` to discover the right-side topic, resolves it via the caller-supplied closure (server registry lookup in production; in-memory `Arc` lookup in tests), builds the combined schema, parses the query against it, and constructs the view topic. `View::refresh` routes to `execute_join_query` when `query.join` is `Some(_)`. New `spawn_view_runner_joined(view, left_tap, right_tap)` fans both source taps into a single merged channel via `crossbeam_channel::select!`, so a publish on EITHER side wakes the refresh — referential-integrity-style changes on the right side propagate to the view without an explicit left-side touch.
  - **Server wiring** — `cq-server`'s `init_view` calls `peek_join` on the configured view SQL. JOIN views go through `build_view_topic_joined` + `spawn_view_runner_joined`; un-joined views take the single-source path unchanged. Both right and left source topics register view taps so either-side mutations refresh the view.
  - **Tests** (8 unit + 4 view convergence + 1 e2e, all green):
    - `crates/cq-core/src/query.rs::tests`: parser accepts JOIN USING (single-side schema test against the combined schema); rejects LEFT OUTER; rejects ON-clause; `peek_join` extracts (left, right, using) and returns None for un-joined SQL; executor aggregates by right-side `sector` over a positions × securities fixture and gets the per-sector exposure right; inner-join drops left rows whose key has no right match AND right rows with no left match; WHERE predicate filters post-join rows.
    - `crates/cq-core/tests/view_join.rs`: a joined view's initial populate computes the right per-sector totals; a left-side publish into `/positions` after view creation lifts the matching sector's exposure to the new value; a right-side publish into `/securities` after positions already exist drives a refresh that resolves previously-unmatched left rows; deleting the right-side security row makes the corresponding group vanish from the view.
    - `crates/cq-e2e-tests/tests/view_join_e2e.rs`: full cqserver child process with two topics + a `[[views]]` JOIN config, SOW-and-subscribe to the view, snapshot matches the expected per-sector exposures, and a subsequent publish to `/positions` bumps the Tech total via the view subscription.
  - **React demo update**: `clients/react-demo/src/components/PivotPanel.tsx`'s "Underlying cqserver query" preview drops the "JOIN runs client-side; S20 JOIN-views are a follow-up" disclaimer — the SQL is now what the server would actually run (the React client still does the client-side join for the panel itself, but that's an implementation choice on the demo side now, not a server limitation).
  - **Honest scope notes**:
    - **INNER JOIN with USING(...) only** — LEFT/RIGHT OUTER, ON-clause non-equi predicates, and multi-table chains (`A JOIN B JOIN C`) are rejected at parse time. The hash-join executor is the natural place to add LEFT OUTER (just don't drop unmatched left rows), and ON-clause support reduces to evaluating an arbitrary CompiledPredicate against the combined row — both are small follow-ups that didn't fit the demo's needs.
    - **Materialised intermediate** — the executor builds a temporary `ColumnStore` for the joined rows on every refresh. For 40K × 500 fixture (the demo) this is fast enough; for million-row joins the natural follow-up is to push GROUP BY into the join walk (single-pass aggregator over joined-on-the-fly rows). The current shape trades that throughput for code reuse against the existing aggregate runtime.
    - **No multi-version snapshot across the two reads** — the view's left + right reads happen under separate `state.read()` calls (taken sequentially inside `Topic::execute_join_query`). A concurrent writer can interleave between them, so a refresh may observe a left view ahead of the right (or vice versa). The next refresh corrects it on the following tap event; for at-least-once eventually-consistent aggregates this is the right tradeoff, but applications that need a strict snapshot across both topics should arrange external ordering.
